...
public class DepositoAhorro {
	private static final Logger log = LogManager.getLogger(DepositoAhorro.class);
	...
	public Respuesta registrarDeposioto(DepositoAhorroReq req){
		initialized();
		Respuesta respuesta = new Respuesta();
		
		
		AhorroContrato ahorroContrato = new AhorroContrato();
		ahorroContrato = adao.buscarByCuenta(req.getCuenta());
		
		AhorroPagare ahorroPagares = pdao.buscarByCuenta(req.getCuenta());
		
		Timestamp vFecha = new Timestamp(System.currentTimeMillis());
		
		AhorroMovimiento ahorroMovimiento = new AhorroMovimiento();
		ahorroMovimiento = mdao.buscarById(req.getMovimientoId());
		
		if(ahorroContrato.getSaldo() == 0){
		//if(true){
			AhorroPagare ahorroPagaresNuevo = new AhorroPagare();
			ahorroPagaresNuevo.setCuenta(req.getCuenta());
			...
			ahorroPagaresNuevo.setCreadoPor((int) req.getHeader().getIdUsuario());
			ahorroPagaresNuevo.setFechaCreacion(vFecha);
			ahorroPagaresNuevo.setModificadoPor((int) req.getHeader().getIdUsuario());
		...
		movimientoCaja.setControl(req.getCuentaCredito());
		movimientoCaja.setCajaId(req.getCajaId());
		movimientoCaja.setCajeroId((int) req.getHeader().getIdUsuario());
		movimientoCaja.setFecha(req.getFecha());
		movimientoCaja.setTipoMovId(req.getMovimientoId());
		movimientoCaja.setMonedaId(1);
		movimientoCaja.setMonto(req.getMonto());
		movimientoCaja.setCuenta(req.getCuenta());
		movimientoCaja.setFormaPago(req.getFormaPago());
		movimientoCaja.setObs(req.getObservaciones());
		movimientoCaja.setBancoId(req.getBancoId());
		movimientoCaja.setBancoOrigen(req.getBancoId());
		movimientoCaja.setRegionId((int) req.getHeader().getIdSucursal());
		movimientoCaja.setCreadoPor((int) req.getHeader().getIdUsuario());
		...
		if(req.getMovimientoId() == 2){
			...
			ahorroDeposito.setObservaciones(req.getObservaciones());
			ahorroDeposito.setCreadoPor((int) req.getHeader().getIdUsuario());
			ahorroDeposito.setFechaCreacion(vFecha);
			ahorroDeposito.setModificadoPor((int) req.getHeader().getIdUsuario());
			ahorroDeposito.setFechaModificacion(vFecha);
			
			addao.nuevo(ahorroDeposito);
		}
		
		if(ahorroMovimiento.getSalvoBuenCobro() == null){
			ahorroActualizasaldo(req.getFecha(),req.getCuenta(), req.getMonto(), ahorroMovimiento.getOperacion());
		}else{
		...
		if(req.getMovimientoId() != 38){
			Double  vide = ahorroIde(req.getCuenta(),req.getFecha(),req.getFecha());
			
			if(vide > 0){
				MovimientosCaja movimientoCajaNuevo = new MovimientosCaja();
				movimientoCajaNuevo.setCajaId(req.getCajaId());
				movimientoCajaNuevo.setCajeroId((int) req.getHeader().getIdUsuario());
				...
				movimientoCajaNuevo.setCuenta(req.getCuenta());
				movimientoCajaNuevo.setFormaPago(4);
				movimientoCajaNuevo.setCreadoPor((int) req.getHeader().getIdUsuario());
				movimientoCajaNuevo.setFechaCreacion(vFecha);
				movimientoCajaNuevo.setModificadoPor((int) req.getHeader().getIdUsuario());
				movimientoCajaNuevo.setFechaModificacion(vFecha);
				movimientoCajaNuevo.setObs("ide");
				movimientoCajaNuevo.setRegionId((int) req.getHeader().getIdSucursal());
				
				movimientoCajaNuevo.setMovimientoId(mcdao.nuevo(movimientoCajaNuevo));
				
				AhorroSaldos ahorroSaldos = new AhorroSaldos();
				ahorroSaldos = asdao.buscarByCuenta(req.getCuenta());
	...
	private Date diaHabil(Date fecha, Integer plazo, String string) {
		...
		List<Date> fechasInhabiles = new ArrayList();
