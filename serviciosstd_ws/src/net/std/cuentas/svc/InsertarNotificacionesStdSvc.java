...
@Controller
public class InsertarNotificacionesStdSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(InsertarNotificacionesStdSvc.class);
	...
	@RequestMapping(value="/insertarNotificacionesStd", method=RequestMethod.POST)
	public ResponseEntity<String> procesar(@RequestBody String json){
		...
		if(!ValidaPermisos.valida(Comun._L(Constantes.TRX_NOTIFICACIONES), "TRX_NOTIFICACIONES: ")){
		...
		try{
			RespuestaSVC respuesta = dao.insertaNotificacionesStdDao(Comun._I(req.getCuentaId()), 
					                                                 Comun._T(req.getNotificacion()),
