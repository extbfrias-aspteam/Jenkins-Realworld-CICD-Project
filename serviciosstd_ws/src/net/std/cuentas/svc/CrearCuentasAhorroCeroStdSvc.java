...
@Controller
public class CrearCuentasAhorroCeroStdSvc implements Serializable{
	...
	@RequestMapping(value="/crearCuentaCeroStd", method=RequestMethod.POST)
	public ResponseEntity<String> procesar(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respProd = daoProd.leerProductosCeroStdDao(Comun._T(Constantes.PRODUCTO_AHORRO), Comun._T(Constantes.ALTA_ID));
			...
			/* paso X1. VERIFICA QUE LA CUENTA NO EXISTA*/
			if("".equals(Comun._T(altaSol.getIdentificador()))){
			...
			RespuestaSVC respReferencia = ProcesoGeneraReferencia.procesar(cuenta.getProductoAhorroId(), Comun._I(Constantes.TIPO_REFERENCIA));
			...
			cuenta.setEstatusId(estatusAhSegID);
