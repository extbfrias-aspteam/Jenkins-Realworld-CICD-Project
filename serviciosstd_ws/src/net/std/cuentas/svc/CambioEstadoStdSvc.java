...
@Controller
public class CambioEstadoStdSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CambioEstadoStdSvc.class);
	...
	@RequestMapping(value="/cambioEstadoStd", method=RequestMethod.POST)
	public ResponseEntity<String> cambioEstadoStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> mapResultado = new HashMap<>();
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		CambioEstadoCuentaReq req = null;
		...
		try{
			...
			req = new Gson().fromJson(json, CambioEstadoCuentaReq.class);
		...
		try{
			...
			RespuestaSVC respCuenta = dao.cambiaEstadoCuentaStdDao(req);
			if(respCuenta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.descGson(Errores.ERROR_ESTADO_CUENTA), HttpStatus.FORBIDDEN);
				return response;
			}
	...
	private String validaParams(CambioEstadoCuentaReq req){
		String valida = null;
