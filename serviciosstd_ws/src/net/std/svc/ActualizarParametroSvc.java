...
@Controller
public class ActualizarParametroSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ActualizarParametroSvc.class);
	...
	@RequestMapping(value="/actualizarParametroPStd", method=RequestMethod.POST)
	public ResponseEntity<String> actualizarParametroPStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/actualizarParametroPStd :: %s", json));
		...
		try{
			...
			if(map == null || map.get("clave") == null || map.get("estatus") == null || map.get("usuarioID") == null){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CAMPOS_REQUERIDOS, "clave|estatus|usuarioID"), HttpStatus.FORBIDDEN);
				return response;
			}
		}catch(Exception ex){
		...
		try{
			RespuestaSVC respuestaSvc = dao.actualizarParametroPStdDao(Comun._T(map.get("clave")), 
