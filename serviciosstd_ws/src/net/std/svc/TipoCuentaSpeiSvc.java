...
@Controller
public class TipoCuentaSpeiSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(TipoCuentaSpeiSvc.class);
	...
	@RequestMapping(value="/tipoCuentaSpei", method=RequestMethod.POST)
	public ResponseEntity<String> tipoCuentaSpei(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/tipoCuentaSpei :: %s", json));
		...
		try{
			RespuestaSVC respuestaSvc = dao.leerTipoCuentaSpeiDao(Comun._T(map.get("valor")));
	...
	@RequestMapping(value="/tipoCuentaSpeiByID", method=RequestMethod.POST)
	public ResponseEntity<String> tipoCuentaSpeiByID(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/tipoCuentaSpeiByID :: %s", json));
		...
		try{
			RespuestaSVC respuestaSvc = dao.leerTipoCuentaSpeiDaoByID(Comun._T(map.get("valor")));
			if(respuestaSvc.getErrores().getCodigoError() == 0){
				response = new ResponseEntity<>(new Gson().toJson(respuestaSvc), HttpStatus.OK);
			}else{
				response = new ResponseEntity<>(respuestaSvc.getErrores().getDescError(), HttpStatus.NOT_FOUND);
