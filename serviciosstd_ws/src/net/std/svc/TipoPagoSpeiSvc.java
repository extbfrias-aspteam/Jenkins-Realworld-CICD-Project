...
@Controller
public class TipoPagoSpeiSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(TipoPagoSpeiSvc.class);
	...
	@RequestMapping(value="/tipoPagoSpei", method=RequestMethod.POST)
	public ResponseEntity<String> tipoPagoSpei(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/tipoPagoSpei :: %s", json));
		...
		try{
			RespuestaSVC respuestaSvc = dao.leerTipoPagoSpeiDao(ToolsR._T(map.get("valor")));
