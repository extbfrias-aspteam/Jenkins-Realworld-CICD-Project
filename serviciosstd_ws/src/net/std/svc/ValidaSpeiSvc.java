...
@Controller
public class ValidaSpeiSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ValidaSpeiSvc.class);
	...
	@RequestMapping(value="/validaSpeiOutgoingStd", method=RequestMethod.POST)
	public ResponseEntity<String> procesar(@RequestBody String json){
		...
		try{
			...
			if(map == null || map.get("claveRastreo") == null){
		...
		try{
			String claveRastreo = Comun._T(map.get("claveRastreo"));
			RespuestaSVC respuestaSvc = dao.validarSpeiOutgoingStdDao(claveRastreo);
