...
@Controller
public class SpeiOutgoingSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(SpeiOutgoingSvc.class);

	@Autowired
	protected IPAuthenticationProvider authenticationManager;
	private static Apps apps = null;
	private static SpeiStdDAO dao = null;
	...
	private static void initialized() {
		...
		try {
			...
			dao = (SpeiStdDAO)s.getApplicationContext().getBean("SpeiStdDAO");
	...
	@RequestMapping(value="/datosOutgoingStd", method=RequestMethod.POST)
	public ResponseEntity<String> tipoPagoSpei(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/datosOutgoingStd :: %s", json));
		...
		try{
			...
			if(map == null || map.get("claveRastreo") == null || map.get("cuentaOrdenante") == null || map.get("cuentaBeneficiario") == null || map.get("monto") == null){
		...
		try{
			RespuestaSVC respuestaSvc = dao.consultaOutgoingPorCveRastreoStdDao(map);
