...
@Controller
public class ParametroSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ParametroSvc.class);
	...
	@RequestMapping(value="/obtenerParametroPStd", method=RequestMethod.POST)
	public ResponseEntity<String> obtenerParametroPStd(@RequestBody String json){
		...
		try{
			...
			if(map == null || map.get("clave") == null){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CAMPOS_REQUERIDOS, "clave"), HttpStatus.FORBIDDEN);
				return response;
			}
		}catch(Exception ex){
		...
		try{
			RespuestaSVC respuestaSvc = dao.parametroPStdDao(Comun._T(map.get("clave")));
