...
@Controller
public class TipoInstitucionSpeiSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(TipoInstitucionSpeiSvc.class);
	...
	@RequestMapping(value="/tipoInstitucionSpei", method=RequestMethod.POST)
	public ResponseEntity<String> tipoInstitucionSpei(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/tipoInstitucionSpei :: %s", json));
		...
		try{
			RespuestaSVC respuestaSvc = dao.leerInstitucionSpeiDao(ToolsR._T(map.get("valor")));
	...
	@RequestMapping(value="/tipoInstitucionSpeiByID", method=RequestMethod.POST)
	public ResponseEntity<String> tipoInstitucionSpeiByID(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/tipoInstitucionSpeiByID :: %s", json));
		...
		try{
			RespuestaSVC respuestaSvc = dao.leerInstitucionSpeiDaoByID(ToolsR._T(map.get("valor")));
			if(respuestaSvc.getErrores().getCodigoError() == 0){
				response = new ResponseEntity<>(new Gson().toJson(respuestaSvc), HttpStatus.OK);
			}else{
				response = new ResponseEntity<>(respuestaSvc.getErrores().getDescError(), HttpStatus.NOT_FOUND);
