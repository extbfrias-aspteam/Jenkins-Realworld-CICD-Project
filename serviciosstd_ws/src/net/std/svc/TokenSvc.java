...
@Controller
public class TokenSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(TokenSvc.class);
	...
	@RequestMapping(value="/obtenerToken", method=RequestMethod.POST)
	public ResponseEntity<String> obtenerToken(@RequestBody String json){
		Thread.currentThread().setName("obtenerToken_"+System.currentTimeMillis());
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/obtenerToken :: %s", json));
		...
		try{
			...
			if(map == null || map.get("fecha") == null || map.get("estatus") == null){
		...
		try{
			RespuestaSVC respuestaSvc = dao.tokenDao(Comun._T(map.get("fecha")), Comun._T(map.get("estatus")));
