...
@Controller
public class InsertarBitacoraSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(InsertarBitacoraSvc.class);

	@Autowired
	protected IPAuthenticationProvider authenticationManager;
	private static Apps apps = null;
	private static BitacoraStdDAO dao = null;
	...
	private static void initialized() {
		...
		try {
			...
			dao = (BitacoraStdDAO)s.getApplicationContext().getBean("BitacoraStdDAO");
	...
	@RequestMapping(value="/insertarBitacoraPStd", method=RequestMethod.POST)
	public ResponseEntity<String> insertarBitacoraPStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/insertarBitacoraPStd :: %s", json));
		...
		try{
			...
			if(map.get("userID") == null || map.get("logger") == null || map.get("level") == null || map.get("log") == null){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CAMPOS_REQUERIDOS, "userID|logger|level|log"), HttpStatus.FORBIDDEN);
				return response;
			}
		}catch(Exception ex){
		...
		try{
			RespuestaSVC respuestaSvc = dao.insertarbitacoraDao(Comun._T(map.get("userID")),  Comun._T(map.get("logger")),
					                                            Comun._T(map.get("level")), Comun._T(map.get("log")));
