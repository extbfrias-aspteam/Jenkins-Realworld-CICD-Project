...
@Controller
public class CatStdPaisSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdPaisSvc.class);
	...
	@RequestMapping(value="/catPrPaisStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrPaisStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getPaisesPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "PAISES"), HttpStatus.NOT_FOUND);
				return response;
			}
