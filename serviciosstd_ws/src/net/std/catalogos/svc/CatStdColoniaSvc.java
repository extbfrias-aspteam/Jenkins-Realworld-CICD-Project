...
@Controller
public class CatStdColoniaSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdColoniaSvc.class);
	...
	@RequestMapping(value="/catPrColoniaStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrColoniaStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getColoniasPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "COLONIAS"), HttpStatus.NOT_FOUND);
				return response;
			}
