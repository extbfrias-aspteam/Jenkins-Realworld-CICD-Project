...
@Controller
public class CatStdMonedasSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdMonedasSvc.class);
	...
	@RequestMapping(value="/catMonedaStd", method=RequestMethod.POST)
	public ResponseEntity<String> catMonedaStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.catMonedasStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "MONEDAS"), HttpStatus.NOT_FOUND);
				return response;
			}
