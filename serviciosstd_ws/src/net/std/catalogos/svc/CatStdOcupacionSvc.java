...
@Controller
public class CatStdOcupacionSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdOcupacionSvc.class);
	...
	@RequestMapping(value="/catPrOcupacionStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrOcupacionStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getOcupacionesPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "OCUPACIONES"), HttpStatus.NOT_FOUND);
				return response;
			}
