...
@Controller
public class CatStdDomicilioSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdDomicilioSvc.class);
	...
	@RequestMapping(value="/catPrDomicilioStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrDomicilioStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getDomiciliosPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "DOMICILIOS"), HttpStatus.NOT_FOUND);
				return response;
			}
