...
@Controller
public class CatStdEstatusAhorroSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdEstatusAhorroSvc.class);
	...
	@RequestMapping(value="/catEstatusAhorroStd", method=RequestMethod.POST)
	public ResponseEntity<String> catEstatusAhorroStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.catEstatusAhorroStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "ESTATUS AHORRO"), HttpStatus.NOT_FOUND);
				return response;
			}
