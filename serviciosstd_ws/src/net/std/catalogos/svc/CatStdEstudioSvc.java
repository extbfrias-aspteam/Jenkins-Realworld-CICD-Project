...
@Controller
public class CatStdEstudioSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdEstudioSvc.class);
	...
	@RequestMapping(value="/catPrEstudioStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrEstudioStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getEstudiosPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "ESTUDIOS"), HttpStatus.NOT_FOUND);
				return response;
			}
