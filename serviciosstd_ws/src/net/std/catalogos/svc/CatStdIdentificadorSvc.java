...
@Controller
public class CatStdIdentificadorSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdIdentificadorSvc.class);
	...
	@RequestMapping(value="/catPrIdentificadorStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrIdentificadorStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getIdentificadoresPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "IDENTIFICADORES"), HttpStatus.NOT_FOUND);
				return response;
			}
