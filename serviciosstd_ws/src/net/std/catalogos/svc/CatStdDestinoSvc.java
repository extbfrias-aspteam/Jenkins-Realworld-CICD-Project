...
@Controller
public class CatStdDestinoSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdDestinoSvc.class);
	...
	@RequestMapping(value="/catPrDestinoStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrDestinoStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getDestinosPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "DESTINOS"), HttpStatus.NOT_FOUND);
				return response;
			}
