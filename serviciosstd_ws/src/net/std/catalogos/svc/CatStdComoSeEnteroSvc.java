...
@Controller
public class CatStdComoSeEnteroSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdComoSeEnteroSvc.class);
	...
	@RequestMapping(value="/catPrComoSeEnteroStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrComoSeEnteroStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.catComoSeEnteroStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "COMO SE ENTERO"), HttpStatus.NOT_FOUND);
				return response;
			}
