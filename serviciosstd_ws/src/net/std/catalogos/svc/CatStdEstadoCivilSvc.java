...
@Controller
public class CatStdEstadoCivilSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdEstadoCivilSvc.class);
	...
	@RequestMapping(value="/catPrEstadoCivilStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrEstadoCivilStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getEstadoCivilPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "ESTADO CIVIL"), HttpStatus.NOT_FOUND);
				return response;
			}
