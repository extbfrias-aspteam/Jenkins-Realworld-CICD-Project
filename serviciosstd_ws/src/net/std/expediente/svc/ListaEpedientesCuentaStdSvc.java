...
import net.std.request.ListExpedientesCuentaReq;

@Controller
public class ListaEpedientesCuentaStdSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ListaEpedientesCuentaStdSvc.class);
	...
	@RequestMapping(value="/listaExpedientesCuentaStd", method=RequestMethod.POST)
	public ResponseEntity<String> listaExpedientesCuentaStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		ListExpedientesCuentaReq req = null;
		...
		try{
			...
			req = new Gson().fromJson(json, ListExpedientesCuentaReq.class);
		...
		try{
			RespuestaSVC respuesta = dao.listExpedienteStdDao(req.getCuenta_id());
			...
			response = new ResponseEntity<>(new Gson().toJson(respuesta.getBody().getValor("EXPEDIENTE")), HttpStatus.OK);
		}catch(Exception ex){
			ex.printStackTrace();
			response = new ResponseEntity<>(Errores.desc(Errores.ERROR_INESPERADO, ex.getMessage()), HttpStatus.BAD_REQUEST);
		}
	...
	private String validaParams(ListExpedientesCuentaReq req){
		String valida = null;

		if(req == null) return ErrProd.desc(ErrProd.ERROR_PARAMETROS, "SIN PARAMETROS");
		if(req.getCuenta_id() == null) return ErrProd.desc(ErrProd.ERROR_PARAMETROS, "NO SE PROPORCIONA ID DE LA CUENTA");
		return valida;
	}
}
