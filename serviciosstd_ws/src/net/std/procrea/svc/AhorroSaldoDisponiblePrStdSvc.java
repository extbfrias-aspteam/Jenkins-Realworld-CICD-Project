...
@Controller
public class AhorroSaldoDisponiblePrStdSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(AhorroSaldoDisponiblePrStdSvc.class);
	...
	@RequestMapping(value="/ahorroSaldoDisponiblePrStd", method=RequestMethod.POST)
	public ResponseEntity<String> ahorroSaldoDisponiblePrStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/ahorroSaldoDisponiblePrStd :: %s", json));
		...
		try{
			...
			if(map == null || map.get("cuenta") == null){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CAMPOS_REQUERIDOS, "cuenta"), HttpStatus.FORBIDDEN);
				return response;
			}
		}catch(Exception ex){
		...
		if(!ValidaPermisos.valida(Comun._L(Constantes.TRX_SALDO_AHORRO), "TRX_SALDO_AHORRO: " + Comun._T(map.get("cuenta")))){
		...
		try{
			RespuestaSVC respuestaSvc = dao.ahorroSaldoDisponiblePrStdDao(Comun._T(map.get("cuenta")));
