...
@Controller
public class CatStdLocalidadSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdLocalidadSvc.class);
	...
	@RequestMapping(value="/catPrLocalidadStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrLocalidadStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getLocalidadesPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "LOCALIDADES"), HttpStatus.NOT_FOUND);
				return response;
			}
