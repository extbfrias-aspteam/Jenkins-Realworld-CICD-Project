...
@Controller
public class CatStdEstatusSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdEstatusSvc.class);
	...
	@RequestMapping(value="/catEstatusStd", method=RequestMethod.POST)
	public ResponseEntity<String> catEstatusStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.catEstatusStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "ESTATUS"), HttpStatus.NOT_FOUND);
				return response;
			}
