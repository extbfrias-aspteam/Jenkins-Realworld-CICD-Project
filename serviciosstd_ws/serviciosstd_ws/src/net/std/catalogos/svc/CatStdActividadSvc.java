...
@Controller
public class CatStdActividadSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdActividadSvc.class);
	...
	@RequestMapping(value="/catPrActividadStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrActividadStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.getActividadesPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "ACTIVIDADES"), HttpStatus.NOT_FOUND);
				return response;
			}
