...
@Controller
public class CatStdTipoIdentificadorSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdTipoIdentificadorSvc.class);
	...
	@RequestMapping(value="/catPrTipoIdentificadorStd", method=RequestMethod.POST)
	public ResponseEntity<String> catPrTipoIdentificadorStd(@RequestBody String json){
		...
		try{
			String clave = "".equals(Comun._T(map.get("clave"))) ? null : Comun._T(map.get("clave"));
			RespuestaSVC respuesta = dao.getTipoIdentificadoresPrStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "TIPO IDENTIFICADORES"), HttpStatus.NOT_FOUND);
				return response;
			}
