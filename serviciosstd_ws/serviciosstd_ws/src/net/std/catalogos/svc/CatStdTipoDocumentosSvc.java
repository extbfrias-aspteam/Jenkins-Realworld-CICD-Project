...
@Controller
public class CatStdTipoDocumentosSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(CatStdTipoDocumentosSvc.class);
	...
	@RequestMapping(value="/catTipoDocumentosStd", method=RequestMethod.POST)
	public ResponseEntity<String> catTipoDocumentosStd(@RequestBody String json){
		...
		try{
			...
			RespuestaSVC respuesta = dao.catTipoDocumentosStdStdDao(clave);
			if(respuesta.getErrores().getCodigoError() != 0){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CATALOGO, "TIPO DOCUMENTOS"), HttpStatus.NOT_FOUND);
				return response;
			}
