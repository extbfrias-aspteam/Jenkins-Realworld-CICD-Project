...
@Controller
public class AhorroRetiroPrStdSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(AhorroRetiroPrStdSvc.class);
	...
	@RequestMapping(value="/ahorroRetiroPrStd", method=RequestMethod.POST)
	public ResponseEntity<String> ahorroRetiroPrStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/ahorroRetiroPrStd :: %s", json));
		...
		if(!ValidaPermisos.valida(Comun._L(Constantes.TRX_RETIRO_AHORRO), "TRX_RETIRO_AHORRO: " + Comun._T(map.get("cuentaOri")))){
		...
		try{
			RespuestaSVC respuestaSvc = dao.ahorroRetiroPrStdDao(
