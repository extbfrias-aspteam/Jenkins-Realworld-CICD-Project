...
@Controller
public class InsertarParametroSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(InsertarParametroSvc.class);

	@Autowired
	protected IPAuthenticationProvider authenticationManager;
	private static Apps apps = null;
	private static ParametrosStdDAO dao = null;
	...
	private static void initialized() {
		...
		try {
			...
			dao = (ParametrosStdDAO)s.getApplicationContext().getBean("ParametrosStdDAO");
	...
	@RequestMapping(value="/insertarParametroPStd", method=RequestMethod.POST)
	public ResponseEntity<String> insertarParametroPStd(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		Map<String, String> map;
		log.info(String.format("/insertarParametroPStd :: %s", json));
		...
		try{
			...
			if(map == null || map.get("clave") == null || map.get("valor") == null || map.get("estatus") == null || map.get("usuarioID") == null){
				response = new ResponseEntity<>(Errores.desc(Errores.ERROR_CAMPOS_REQUERIDOS, "clave|valor|estatus|usuarioID"), HttpStatus.FORBIDDEN);
				return response;
			}
		}catch(Exception ex){
		...
		try{
			RespuestaSVC respuestaSvc = dao.insertarParametroPStdDao(Comun._T(map.get("clave")), 
