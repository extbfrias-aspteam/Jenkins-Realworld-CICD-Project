...
import net.std.request.ListCuentasProdAhorroReq;

@Controller
public class ListCuentasProductoAhorroSvc implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ListCuentasProductoAhorroSvc.class);
	...
	@RequestMapping(value="/lstCuentaProductoAhorroStd", method=RequestMethod.POST)
	public ResponseEntity<String> procesar(@RequestBody String json){
		SecurityContext securityContext = SecurityContextHolder.getContext();
		ResponseEntity<String> response = null;
		ListCuentasProdAhorroReq req = null;
		...
		try{
			...
			req = new Gson().fromJson(json, ListCuentasProdAhorroReq.class);
		...
		try{
			RespuestaSVC respuesta = dao.listCuentaProductoAhorroStdDao(req.getTipoProducto(), req.getProductoAhorroId());
			...
			response = new ResponseEntity<>(new Gson().toJson(respuesta.getBody().getValor("CUENTA")), HttpStatus.OK);
		}catch(Exception ex){
			ex.printStackTrace();
			response = new ResponseEntity<>(Errores.desc(Errores.ERROR_INESPERADO, ex.getMessage()), HttpStatus.BAD_REQUEST);
		}
	...
	private String validaParams(ListCuentasProdAhorroReq req){
		String valida = null;

		if(req == null) return ErrProd.desc(ErrProd.ERROR_INESPERADO, "SIN PARAMETROS");
		if("".equals(req.getTipoProducto())) return ErrProd.desc(ErrProd.ERROR_PARAMETROS, "NO SE PROPORCIONA TIPO DE CONSULTA");
		if(req.getProductoAhorroId() == null) return ErrProd.desc(ErrProd.ERROR_PARAMETROS, "NO SE PROPORCIONA EL TIPO DE PRODUCTO");
