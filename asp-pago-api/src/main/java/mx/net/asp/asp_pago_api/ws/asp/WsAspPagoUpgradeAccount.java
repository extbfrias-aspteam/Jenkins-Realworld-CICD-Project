...
@Service
@Log4j2
public class WsAspPagoUpgradeAccount {
    @Value("${ms.asp.pago.services.host}")
    private String msAspPagoServicesHost;
    @Value("${ms.asp.pago.services.port}")
    private String msAspPagoServicesPort;
    @Value("${asp.pago.services.user}")
    private String msAspPagoServicesUser;
    @Value("${asp.pago.services.pass}")
    private String msAspPagoServicesPass;
    private final InvokeRestServiceUtil invokeRestServiceUtil;

    @Autowired
    public WsAspPagoUpgradeAccount(InvokeRestServiceUtil invokeRestServiceUtil) {
    ...
    public RespuestaDTO enviarPeticion(String pathEndPoint, Object object, HttpMethod method, Map<String, String> queryParams) {
        ...
        String url = InvokeRestServiceUtil.getURLRest(msAspPagoServicesHost, msAspPagoServicesPort, pathEndPoint);
    ...
    private String getAuthorizationHeader() {
        String auth = msAspPagoServicesUser + ":" + msAspPagoServicesPass;
