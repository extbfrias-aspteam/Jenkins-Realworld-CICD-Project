...
@Service
@Log4j2
public class WsAspPagoOnb {
    @Value("${ms.onb.host}")
    private String msOnbHost;
    @Value("${ms.onb.port}")
    private String msOnbPort;
    @Value("${asp.pago.onb.user}")
    private String msOnbUser;
    @Value("${asp.pago.onb.pass}")
    private String msOnbPass;
    private final InvokeRestServiceUtil invokeRestServiceUtil;

    @Autowired
    public WsAspPagoOnb(InvokeRestServiceUtil invokeRestServiceUtil) {
    ...
    public RespuestaDTO enviarPeticion(String pathEndPoint, Object object, HttpMethod method, Map<String, String> queryParams) {
        ...
        String url = InvokeRestServiceUtil.getURLRest(msOnbHost, msOnbPort, pathEndPoint);
    ...
    private String getAuthorizationHeader() {
        String auth = msOnbUser + ":" + msOnbPass;
