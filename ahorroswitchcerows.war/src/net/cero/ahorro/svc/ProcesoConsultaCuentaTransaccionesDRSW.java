...
@Controller
public class ProcesoConsultaCuentaTransaccionesDRSW implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ProcesoConsultaCuentaTransaccionesDRSW.class);


	@Autowired
	protected IPAuthenticationProvider authenticationManager;

	@RequestMapping(value="/consultaCuentaTransaccionesDRSW", method=RequestMethod.POST)
	public ResponseEntity<String> consultaCuentaTransaccionesDRSW(@RequestBody String json){
		...
		try{
			respuestaSvc = ServiciosAhorroWS.listaCuentaTransaccionesDRSW(map);

			resp.setCodigo(0);
			resp.setMensaje("OK");
			resp.setData(gson.toJson(respuestaSvc));
		}catch(DataAccessException ex){
			log.error("Error [consultaCuentaTransaccionesDRSW] : ", ex);
			response = new ResponseEntity<>(HttpStatus.BAD_REQUEST);
			return response;
		}
