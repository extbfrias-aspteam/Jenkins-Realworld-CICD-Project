...
import net.cero.ahorro.servicios.ServicioRetiroAhorro;
...
@Controller
public class ProcesoRetirarAhorro implements Serializable {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ProcesoRetirarAhorro.class);

	private final ServicioRetiroAhorro logic;
	private final ServicioOperacionesTarjetaDock servicioOperacionesTarjetaDock;

	@Autowired
	protected IPAuthenticationProvider authenticationManager;

	public ProcesoRetirarAhorro(ServicioRetiroAhorro logic, ServicioOperacionesTarjetaDock servicioOperacionesTarjetaDock) {
		this.logic = logic;
		this.servicioOperacionesTarjetaDock = servicioOperacionesTarjetaDock;
	}

	@RequestMapping(value = "/retirarAhorro", method = RequestMethod.POST)
	public ResponseEntity<String> retirarAhorro(@RequestBody String json, HttpServletRequest request) {
		...
		try{
			...
			if (map.get("cuenta") == null || map.get("tipoTransaccionID") == null || map.get("tipoClave") == null
					|| map.get("fecha") == null || map.get("monto") == null || map.get("descripcion") == null
					|| map.get("estatusID") == null) {
			...
			if (!"OK".equals(valMonto)) {
				resp.setMensaje("MONTO A RETIRAR INCORRECTO");
				jsonResponse = gson.toJson(resp);
				return new ResponseEntity<>(jsonResponse, HttpStatus.BAD_REQUEST);
			}
			...
			try {
				...
				respuestaSvc = "SI".equals(estatus) && !StringUtils.isBlank(tarjeta) ? logic.retirarProveedor(map) : logic.retirarLocal(map);
				...
				log.error("ERROR: PROCESO DE RETIRO INCORRECTO",ex);
				resp.setMensaje("ERROR: PROCESO DE RETIRO INCORRECTO");
				jsonResponse = gson.toJson(resp);
	...
	private String validarCuenta(RespuestaSVC resp) {
		if ("".equals(ToolsR._T(resp.getBody().getValor("CUENTA"))))
			return "INVALIDA";
		if (!"ALTA".equals(ToolsR._T(resp.getBody().getValor("ESTATUS"))))
	...
	private String validarMonto(Map<String, Object> map) {
		if ("".equals(ToolsR._T(map.get("monto"))))
			return "MONTO INVALIDO";
		if (ToolsR._D(map.get("monto")).doubleValue() <= 0.00d)
			return "MONTO CERO O INFERIOR";
		return "OK";
	}

	/*********************************************************
	 * Filtra el valor maximo permitido, si ocurre un error, el filtro no deja
	 * pasar
	 * 
	 * @param cuenta
	 * @param concepto
