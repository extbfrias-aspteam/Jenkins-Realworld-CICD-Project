...
public class ServiciosBusquedaSolicitanteWS implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = LogManager.getLogger(ServiciosBusquedaSolicitanteWS.class);

	public static RespuestaSVC procesa(String solicitanteID) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		Gson gson = ToolsR.GBuilder();
		String uri = new StrBuilder(Constantes.AHORRO_SIMPLIFICADA_WS).append("/").append("solicitanteCS").toString();
		log.info(uri);
		...
		if("".equals(ToolsR._T(solicitanteID))){
			respuestaSvc.getErrores().addCodigo("GENERICO", Errores.ERROR_INESPERADO, "IDENTIFICADOR INCOMPLETO");
			return respuestaSvc;
		}
		
		Map<String, Object> map = new HashMap<>();
		map.put("SOLICITANTE_ID", solicitanteID);
