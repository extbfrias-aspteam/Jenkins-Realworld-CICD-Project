...
public class ServiciosCuentasReferenciaPrWS implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = LogManager.getLogger(ServiciosCuentasReferenciaPrWS.class);

	public static RespuestaSVC leerCuentaReferenciaAhorroPrSW(Map<String, Object> map) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		Gson gson = ToolsR.GBuilder();
		String uri = new StrBuilder(Constantes.AHORRO_SIMPLIFICADA_WS).append("/").append("leerCuentaReferenciaAhorroCS").toString();
		log.info(uri);
		...
		try{
			...
			respuestaSvc.getErrores().addCodigo("GENERICO", Errores.ERROR_INESPERADO, "leerCuentaReferenciaAhorroPrSW");
		}
		return respuestaSvc;
	}
}
