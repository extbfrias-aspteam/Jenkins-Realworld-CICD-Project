...
public class ServiciosSaldoDisponiblePrWS implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = LogManager.getLogger(ServiciosSaldoDisponiblePrWS.class);

	public static RespuestaSVC leerSaldoDisponiblePrSW(Map<String, Object> map) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		Gson gson = ToolsR.GBuilder();
		String uri = new StrBuilder(Constantes.AHORRO_SIMPLIFICADA_WS).append("/").append("leerSaldoDisponibleCS").toString();
		log.info(uri);
		...
		try{
			...
			respuestaSvc.getErrores().addCodigo("GENERICO", Errores.ERROR_INESPERADO, "leerSaldoDisponiblePrSW");
		}
		return respuestaSvc;
	}
}
