...
public class ServiciosCajaDisposicionPrWS implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = LogManager.getLogger(ServiciosCajaDisposicionPrWS.class);

	public static RespuestaSVC cajaDisposicionAhorroPrSW(Map<String, Object> map) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		Gson gson = ToolsR.GBuilder();
		String uri = new StrBuilder(Constantes.AHORRO_SIMPLIFICADA_WS).append("/").append("cajaDisposicionAhorroCS").toString();
		log.info(uri);
		...
		try{
			...
			respuestaSvc.getErrores().addCodigo("GENERICO", Errores.ERROR_INESPERADO, "cajaDisposicionAhorroPrSW");
		}
		return respuestaSvc;
	}
}
