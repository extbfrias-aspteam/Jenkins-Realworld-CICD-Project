...
public class ServiciosCuentasMultiplesPrWS implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = LogManager.getLogger(ServiciosCuentasMultiplesPrWS.class);

	public static RespuestaSVC leerCuentaMultipleAhorroPrSW(Map<String, Object> map) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		Gson gson = ToolsR.GBuilder();
		String uri = new StrBuilder(Constantes.AHORRO_SIMPLIFICADA_WS).append("/").append("leerCuentaMultipleAhorroCS").toString();
		log.info(uri);
		...
		try{
			...
			respuestaSvc.getErrores().addCodigo("GENERICO", Errores.ERROR_INESPERADO, "leerCuentaMultipleAhorroPrSW");
		}
		return respuestaSvc;
	}
}
