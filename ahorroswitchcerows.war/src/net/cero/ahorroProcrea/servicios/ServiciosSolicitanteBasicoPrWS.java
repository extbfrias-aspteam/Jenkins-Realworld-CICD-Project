...
public class ServiciosSolicitanteBasicoPrWS implements Serializable {
	private static final long serialVersionUID = 1L;
	private static Logger log = LogManager.getLogger(ServiciosSolicitanteBasicoPrWS.class);

	
	public static RespuestaSVC leerSolicitanteBasicoPrSW(Map<String, Object> map) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		Gson gson = ToolsR.GBuilder();
		String uri = new StrBuilder(Constantes.AHORRO_SIMPLIFICADA_WS).append("/").append("leerSolicitanteBasicoCS").toString();
		log.info(uri);
		...
		try{
			...
			respuestaSvc.getErrores().addCodigo("GENERICO", Errores.ERROR_INESPERADO, "leerSolicitanteBasicoPrSW");
		}
		return respuestaSvc;
	}
}
