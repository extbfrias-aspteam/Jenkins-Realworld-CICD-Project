...
import net.cero.ahorroProcrea.servicios.ServiciosCuentasMultiplesPrWS;
...
@Controller
public class ProcesoLeerCuentaUnicaAhorroPrSW implements Serializable{
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(ProcesoLeerCuentaUnicaAhorroPrSW.class);

	@Autowired
	protected IPAuthenticationProvider authenticationManager;

	@SuppressWarnings("unchecked")
	@RequestMapping(value="/procesoLeerCuentaUnicaAhorroPrSW", method=RequestMethod.POST)
	public ResponseEntity<String> procesoLeerCuentaUnicaAhorroPrSW(@RequestBody String json){
		...
		try{
			RespuestaSVC respuestaCta = ServiciosCuentasMultiplesPrWS.leerCuentaMultipleAhorroPrSW(map);
			if(respuestaCta.getErrores().getCodigoError() == 0){
				List<Map<String, String>> list = (List<Map<String, String>>)respuestaCta.getBody().getValor("CUENTAS_PROCREA");
				if(list != null){
