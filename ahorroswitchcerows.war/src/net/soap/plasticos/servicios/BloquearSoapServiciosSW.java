...
import net.cero.plastico.bloquear.BloquearIfz;
import net.cero.plastico.bloquear.BloquearIfzService;
import net.cero.plastico.bloquear.BloquearRequest;
import net.cero.plastico.bloquear.BloquearResponse;
 ...
 *
 */
public class BloquearSoapServiciosSW {
	private static final Logger log = LogManager.getLogger(BloquearSoapServiciosSW.class);

	public static RespuestaSVC Bloquear(PlaHeaderWS header, DatosPlasticoREQ pla) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		URL wsdl = null;
		DatosPlasticoOBJ obj = null;

		try{
			wsdl = new URL(BloquearIfzService.WSDL_LOCATION.toString().replace("localhost", Constantes.SERVICIOS_SISCOOP));
		}catch(Exception ex){
			log.error(ex);
		...
		try{
			BloquearIfzService service1 = new BloquearIfzService(wsdl);
			BloquearIfz port1 = service1.getBloquearIfzPort();

			BloquearRequest req = new BloquearRequest();
			req.setHeader(header);
			req.setDatosPlasticoREQ(pla);
			BloquearResponse response = port1.procesar(req);
