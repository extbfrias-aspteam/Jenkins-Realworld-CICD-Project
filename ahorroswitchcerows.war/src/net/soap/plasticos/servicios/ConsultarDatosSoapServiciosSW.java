...
import net.cero.plastico.consultardatos.ConsultarDatosIfz;
import net.cero.plastico.consultardatos.ConsultarDatosIfzService;
import net.cero.plastico.consultardatos.ConsultarDatosRequest;
import net.cero.plastico.consultardatos.ConsultarDatosResponse;
 ...
 *
 */
public class ConsultarDatosSoapServiciosSW {
	private static final Logger log = LogManager.getLogger(ConsultarDatosSoapServiciosSW.class);

	public static RespuestaSVC ConsultarDatos(PlaHeaderWS header, DatosPlasticoREQ pla) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		URL wsdl = null;
		DatosPlasticoOBJ obj = null;

		try{
			wsdl = new URL(ConsultarDatosIfzService.WSDL_LOCATION.toString().replace("localhost", Constantes.SERVICIOS_SISCOOP));
		}catch(Exception ex){
			log.error(ex);
		...
		try{
			ConsultarDatosIfzService service1 = new ConsultarDatosIfzService(wsdl);
			ConsultarDatosIfz port1 = service1.getConsultarDatosIfzPort();

			ConsultarDatosRequest req = new ConsultarDatosRequest();
			req.setHeader(header);
			req.setDatosPlasticoREQ(pla);
			ConsultarDatosResponse response = port1.procesar(req);
