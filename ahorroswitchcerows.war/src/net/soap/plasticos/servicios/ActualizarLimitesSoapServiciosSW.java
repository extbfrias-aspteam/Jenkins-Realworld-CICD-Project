...
import net.cero.plastico.actualizarlimites.ActualizarLimitesIfz;
import net.cero.plastico.actualizarlimites.ActualizarLimitesIfzService;
import net.cero.plastico.actualizarlimites.ActualizarLimitesRequest;
import net.cero.plastico.actualizarlimites.ActualizarLimitesResponse;
 ...
 *
 */
public class ActualizarLimitesSoapServiciosSW {
	private static final Logger log = LogManager.getLogger(ActualizarLimitesSoapServiciosSW.class);

	public static RespuestaSVC ActualizarLimites(PlaHeaderWS header, DatosPlasticoREQ pla) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		URL wsdl = null;
		DatosPlasticoOBJ obj = null;

		try{
			wsdl = new URL(ActualizarLimitesIfzService.WSDL_LOCATION.toString().replace("localhost", Constantes.SERVICIOS_SISCOOP));
		}catch(Exception ex){
			log.error(ex);
		...
		try{
			ActualizarLimitesIfzService service1 = new ActualizarLimitesIfzService(wsdl);
			ActualizarLimitesIfz port1 = service1.getActualizarLimitesIfzPort();

			ActualizarLimitesRequest req = new ActualizarLimitesRequest();
			req.setHeader(header);
			req.setDatosPlasticoREQ(pla);
			ActualizarLimitesResponse response = port1.procesar(req);
