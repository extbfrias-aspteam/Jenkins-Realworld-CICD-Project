...
import net.cero.plastico.bloqueartemporal.BloquearTemporalIfz;
import net.cero.plastico.bloqueartemporal.BloquearTemporalIfzService;
import net.cero.plastico.bloqueartemporal.BloquearTemporalRequest;
import net.cero.plastico.bloqueartemporal.BloquearTemporalResponse;
 ...
 *
 */
public class DesBloquearTemporalSoapServiciosSW {
	private static final Logger log = LogManager.getLogger(DesBloquearTemporalSoapServiciosSW.class);

	public static RespuestaSVC DesBloquearTemporal(PlaHeaderWS header, DatosPlasticoREQ pla) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		URL wsdl = null;
		DatosPlasticoOBJ obj = null;

		try{
			wsdl = new URL(BloquearTemporalIfzService.WSDL_LOCATION.toString().replace("localhost", Constantes.SERVICIOS_SISCOOP));
		}catch(Exception ex){
			log.error(ex);
		...
		try{
			BloquearTemporalIfzService service1 = new BloquearTemporalIfzService(wsdl);
			BloquearTemporalIfz port1 = service1.getBloquearTemporalIfzPort();

			BloquearTemporalRequest req = new BloquearTemporalRequest();
			pla.setTipoOperacion(2);  //2 = Desbloqueo
			req.setHeader(header);
			req.setDatosPlasticoREQ(pla);
			BloquearTemporalResponse response = port1.procesar(req);
