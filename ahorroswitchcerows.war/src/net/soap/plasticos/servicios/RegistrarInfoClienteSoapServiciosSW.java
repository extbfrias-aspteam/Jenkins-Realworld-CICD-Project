...
import net.cero.plastico.registrarinfocliente.RegistrarInfoClienteIfz;
import net.cero.plastico.registrarinfocliente.RegistrarInfoClienteIfzService;
import net.cero.plastico.registrarinfocliente.RegistrarInfoClienteRequest;
import net.cero.plastico.registrarinfocliente.RegistrarInfoClienteResponse;
 ...
 *
 */
public class RegistrarInfoClienteSoapServiciosSW {
	private static final Logger log = LogManager.getLogger(RegistrarInfoClienteSoapServiciosSW.class);

	public static RespuestaSVC RegistrarInfoCliente(PlaHeaderWS header, DatosPlasticoREQ pla) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		URL wsdl = null;
		DatosPlasticoOBJ obj = null;

		try{
			wsdl = new URL(RegistrarInfoClienteIfzService.WSDL_LOCATION.toString().replace("localhost", Constantes.SERVICIOS_SISCOOP));
		}catch(Exception ex){
			log.error(ex);
		...
		try{
			RegistrarInfoClienteIfzService service1 = new RegistrarInfoClienteIfzService(wsdl);
			RegistrarInfoClienteIfz port1 = service1.getRegistrarInfoClienteIfzPort();

			RegistrarInfoClienteRequest req = new RegistrarInfoClienteRequest();
			req.setHeader(header);
			req.setDatosPlasticoREQ(pla);
			RegistrarInfoClienteResponse response = port1.procesar(req);
