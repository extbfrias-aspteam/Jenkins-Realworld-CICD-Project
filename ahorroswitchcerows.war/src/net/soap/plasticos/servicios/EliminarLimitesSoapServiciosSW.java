...
import net.cero.plastico.eliminarlimites.EliminarLimitesIfz;
import net.cero.plastico.eliminarlimites.EliminarLimitesIfzService;
import net.cero.plastico.eliminarlimites.EliminarLimitesRequest;
import net.cero.plastico.eliminarlimites.EliminarLimitesResponse;
 ...
 *
 */
public class EliminarLimitesSoapServiciosSW {
	private static final Logger log = LogManager.getLogger(EliminarLimitesSoapServiciosSW.class);

	public static RespuestaSVC EliminarLimites(PlaHeaderWS header, DatosPlasticoREQ pla) {
		RespuestaSVC respuestaSvc = new RespuestaSVC();
		URL wsdl = null;
		DatosPlasticoOBJ obj = null;

		try{
			wsdl = new URL(EliminarLimitesIfzService.WSDL_LOCATION.toString().replace("localhost", Constantes.SERVICIOS_SISCOOP));
		}catch(Exception ex){
			log.error(ex);
		...
		try{
			EliminarLimitesIfzService service1 = new EliminarLimitesIfzService(wsdl);
			EliminarLimitesIfz port1 = service1.getEliminarLimitesIfzPort();

			EliminarLimitesRequest req = new EliminarLimitesRequest();
			req.setHeader(header);
			req.setDatosPlasticoREQ(pla);
			EliminarLimitesResponse response = port1.procesar(req);
